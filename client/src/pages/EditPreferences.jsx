...
export default function EditPreferences() {
  ...
  const handleCompanyStageToggle = (value, clearOthers = false) => {
    if (clearOthers) {
      ...
      let newStages;
      if (preferences.companyStages.includes(value)) {
        // Deselecting
        newStages = preferences.companyStages.filter(s => s !== value);
      } else {
        // Selecting - remove "no-preference" if it exists
        newStages = preferences.companyStages.filter(s => s !== "no-preference");
        newStages = [...newStages, value];
      }
  ...
  const handleWorkStyleToggle = (value, clearOthers = false) => {
    if (clearOthers) {
      ...
      let newStyles;
      if (preferences.workStyles.includes(value)) {
        // Deselecting
        newStyles = preferences.workStyles.filter(s => s !== value);
      } else {
        // Selecting - remove "no-preference" if it exists
        newStyles = preferences.workStyles.filter(s => s !== "no-preference");
